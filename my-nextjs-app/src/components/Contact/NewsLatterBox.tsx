...
const NewsLatterBox = () => {
  ...
  const { subscribe, loading, error, success, resetState } = useNewsletter();
    ...
    <div className="relative z-10 rounded-sm bg-white p-8 shadow-three dark:bg-gray-dark sm:p-11 lg:p-8 xl:p-11">
      ...
      <p className="mb-11 border-b border-body-color border-opacity-25 pb-11 text-base leading-relaxed text-body-color dark:border-white dark:border-opacity-25">
       Restez informé des dernières nouvelles et mises à jour.
        Abonnez-vous pour ne rien manquer et profiter des nouveautés en avant-première.
      </p>
      <form onSubmit={handleSubmit}>
        ...
        <input
          type="submit"
          value={loading ? "Envoi..." : "Subscribe"}
          className="mb-5 flex w-full cursor-pointer items-center justify-center rounded-sm bg-primary px-9 py-4 text-base font-medium text-white shadow-submit duration-300 hover:bg-primary/90 dark:shadow-submit-dark"
          disabled={loading}
        />
        {success && (
          <p className="text-green-600 text-center mb-2">Merci pour votre abonnement ! 🎉</p>
        )}
        {error && (
